		...
		okButton.addActionListener(new ActionListener()
        {
            public void actionPerformed(ActionEvent event)
            {
            	...
            	if(gameName.equals("") && recordGameIsEnabled){
            	    ...
            	    gui.console.printWarning("When game recording is selected, you have to enter a game name!");
            		return;
            	}
            	try {
	...
	private String[] createList() {
		...
		if(files != null) {
			for(int i = 0; i < files.length; i++) {
				if(files[i].getName().substring(files[i].getName().length()-5,files[i].getName().length()).equals(".java")){
					listLength++;
				}
			}
		}
		...
		if(files != null) {
			...
			while(j < listLength-1) {
				if(files[j].getName().substring(files[j].getName().length()-5,files[j].getName().length()).equals(".java")){
